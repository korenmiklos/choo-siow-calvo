...
\begin{equation}
\label{eq:mm2}
 \operatorname{Cov}(y_{im}, y_{im'}) = \sigma_a^2 + 2\rho\sigma_a\sigma_z + \rho^2\sigma_z^2 = (\sigma_a + \rho\sigma_z)^2.
\end{equation}
Similarly, for two firms $i$ and $i'$ run by the same manager $m$ (firm--firm link at distance 2),
\begin{equation}
\label{eq:ff2}
 \operatorname{Cov}(y_{im}, y_{i'm}) = \sigma_z^2 + 2\rho\sigma_a\sigma_z + \rho^2\sigma_a^2 = (\sigma_z + \rho\sigma_a)^2.
\end{equation}
...
The path attenuation principle extends to arbitrary even path lengths. For 6-step manager-manager pairs connected by path $m_1 \leftrightarrow i_1 \leftrightarrow m_2 \leftrightarrow i_2 \leftrightarrow m_3 \leftrightarrow i_3 \leftrightarrow m_4$, the observed covariance $\operatorname{Cov}(y_{i_1m_1}, y_{i_3m_4})$ decomposes into four latent covariances: $\operatorname{Cov}(a_{i_1}, a_{i_3}) + \operatorname{Cov}(a_{i_1}, z_{m_4}) + \operatorname{Cov}(z_{m_1}, a_{i_3}) + \operatorname{Cov}(z_{m_1}, z_{m_4})$.\footnote{The random-effects model can be formalized as a bipartite Gaussian Markov random field (6), in which conditional independencies follow the network structure and correlations decay exponentially with graph distance. See SI Appendix for the precision matrix formulation.} By path propagation, firm-firm distance 4 gives $\operatorname{Cov}(a_{i_1}, a_{i_3})=\rho^4\sigma_a^2$; manager-manager distance 6 gives $\operatorname{Cov}(z_{m_1}, z_{m_4})=\rho^6\sigma_z^2$; and the cross-type paths (distance 5) each contribute $\rho^5\sigma_a\sigma_z$. Summing yields
\begin{equation}
\label{eq:mm6}
 \operatorname{Cov}(y_{i_1m_1}, y_{i_3m_4}) = \rho^4\sigma_a^2 + 2\rho^5\sigma_a\sigma_z + \rho^6\sigma_z^2 = \rho^4(\sigma_a + \rho\sigma_z)^2.
\end{equation}
...
\addtabletext{MM 2-step counts manager-manager pairs sharing at least one firm. MM 4-step counts pure 4-step connections after excluding pairs with direct 2-step links.}
\end{table}
...
% NOTE: Need to cite maximum 12 references total (PNAS limit)
% Required citations: 
% - Worker-firm correlation studies (max 3-4): AKM, Andrews et al., [1-2 more]
% - CEO literature (max 3-4): Strategic management/corp finance on CEO effects/R-squared
% - Gregory Clark genetics paper (path-length approach)
% - Own work with Orbán and Telegdy (event study ~20% R-squared, 5-6% causal effects)
% - Misallocation: Hsieh-Klenow classical papers (max 3 total)
% TODO: Comment on fixed-effects approaches overestimating R-squared due to bias
...
Hungarian firm-CEO data come from the tax authority and cover 1992--2021. For each firm-year, we observe the registered CEO (person identifier) and annual revenue. We deflate nominal revenue using standard price indices. If a firm--manager relation spans multiple years within a window, we average log revenues.
...
% TODO: Create SI Appendix with full GMRF formulation (Rue & Held 2005, ref 6)
% SI should include:
% 1. Precision matrix formulation: Q with blocks D_A = σ_a^{-2} I, D_Z = σ_z^{-2} I, off-diagonal -ρW
%    where W shares sparsity pattern of adjacency matrix G
% 2. Conditional independence structure: a_i ⊥ (a_{-i}, z_{J\N(i)}) | z_{N(i)}
% 3. Marginal variance expressions: Var(a_i)=σ_a^2, Var(z_j)=σ_z^2, Cov(a_i,z_j)=ρσ_aσ_z
% 4. Proof that dependence propagates along edges and decays exponentially with path length
% 5. Discussion of how additive noise ε_{ij} makes observed field non-Markovian
% 6. Derivations for 8-step, 10-step moments as additional overidentification tests
% 7. Connection to spatial statistics and network autoregression literature
...
% If overidentification tests reject, consider extensions (heterogeneous ρ, non-Gaussian noise)
